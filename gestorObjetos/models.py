...
class PalabraClave(models.Model):
    ...
    """
    palabra_clave = models.CharField(help_text='Palabra que describe el Objeto', verbose_name="Palabra Clave",
...
class Autor(models.Model):
    ...
    nombres = models.CharField(help_text="Nombres del Autor del Objeto.", verbose_name='Nombres', max_length=100,
                               null=False)
    """Apellidos del Autor del Objeto."""
    apellidos = models.CharField(help_text="Apellidos del Autor del Objeto.", verbose_name='Apellidos', max_length=100,
                                 null=False)
    """Campo para el rol que representa el usuario en el objeto."""
    rol = models.CharField(help_text="Papel que juega en la creación del Objeto", verbose_name='Rol', max_length=100,
                           default="Autor")
...
class RutaCategoria(models.Model):
    ...
    nombre_ruta = models.CharField(help_text="Nombre de la Categoría.", verbose_name='Categoría', max_length=150,
                                   null=False)
    """Relación a la :model:'gestorObjetos.RutaCategoria' para determinar si tiene una categoría padre"""
    cat_padre = models.ForeignKey('self', null=True, blank=True, related_name='+', verbose_name='Categoría Padre',
                                  help_text="Categoría que la contiene")
...
class EspecificacionLOM(models.Model):
    ...
    lc1_titulo = models.CharField(help_text='Nombre para el objeto', verbose_name='Título', max_length=200, unique=True,
                                  null=False)
    """Lenguaje primario o lenguajes predominante en el objeto y que se utiliza para comunicarse con el usuario."""
    lc1_idioma = models.CharField(help_text='Lenguaje predominante en el objeto', verbose_name='Idioma', max_length=2,
                                  choices=opc.get_idiomas(), default=opc.get_idiomas()[0][0])
    """Descripción Textual del contenido del objeto."""
    lc1_descripcion = models.TextField(help_text='Descripción Textual del contenido del objeto',
    ...
    lc2_fecha = models.DateTimeField(help_text='Fecha en que el objeto es publicado',
...
class Objeto(models.Model):
    ...
    publicado = models.BooleanField(help_text='Marca para publicar en repositorio', verbose_name='Publicado',
                                    default=False)
    """Atributo que permite identificar si es un objeto virtual de aprendizaje o recurso digital"""
    tipo_obj = models.CharField(help_text='Tipo de Objeto', verbose_name='Tipo de Objeto', max_length=3,
                                choices=opc.get_tipo_obj(), default=opc.get_tipo_obj()[1][0])
...
class UserProfile(models.Model):
    ...
    rol = models.CharField(help_text='Rol que identifica la participación del :model:"User" en el sistema',
                           verbose_name='Rol de Usuario', max_length=4, choices=opc.get_roles(),
