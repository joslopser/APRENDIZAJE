...
import repositorio.lib.Archivos as archivos
...
from gestorObjetos.forms import CatalogacionExeLearning, RegistarUsuarioForm, EspecificacionForm, cEspecificacionForm, ObjetosForm, cObjetosForm
import datetime
import operator
...
from gestorObjetos.models import UserProfile
    ...
    if 'q' in request.GET and request.GET['q']:
        ...
        if request.user.is_authenticated():
            ...
            for g in grupos:
                ...
                r_obj.extend(Objeto.objects.filter(
                    Q(palabras_claves__palabra_clave__icontains=q) | Q(espec_lom__lc1_titulo__icontains=q) | Q(
                        espec_lom__lc1_descripcion__icontains=q) ).order_by(
                    'espec_lom').filter(publicado=True).filter(repositorio__grupos=g))
        #r_obj.extend(Objeto.objects.filter( Q( palabras_claves__palabra_clave__icontains = q ) | Q( espec_lom__lc1_titulo__icontains = q ) | Q( espec_lom__lc1_descripcion__icontains = q ) | Q( espec_lom__lc4_poblacion__icontains = q ) | Q( espec_lom__lc4_contexto__icontains = q ) | Q( espec_lom__lc6_uso_educativo__icontains = q )).order_by('espec_lom').filter(publicado=True).filter(repositorio__publico=True))
        r_obj.extend(Objeto.objects.filter(
            Q(palabras_claves__palabra_clave__icontains=q) | Q(espec_lom__lc1_titulo__icontains=q) | Q(
                espec_lom__lc1_descripcion__icontains=q) ).order_by(
            'espec_lom').filter(publicado=True).filter(repositorio__publico=True))
        rob=list(set(r_obj))
        for r in rob:
...
@login_required(login_url='/ingresar')
def borrarObjeto(request,id_objeto):
    """
    Vista de acceso a Docente para borrar objetos
    """
    ...
    if request.user.profile.rol == 'rdoc':
        ...
        return HttpResponseRedirect('/')
    return render_to_response('borrarObjeto.html', {'objeto': obj, 'usuario': request.user}, context_instance=RequestContext(request))
    ...
    if request.user.is_anonymous() is True:
        ...
        if request.method == 'POST':
            ...
            if form.is_valid():
                ...
                userprofile = UserProfile.objects.create(rol=dato['RolUsu'], user=ru)
                userprofile.save()

                sitio="sitio.com"
                contenido='BIENVENIDO AL REPOSITORIO DE OBJETOS DE APRENDIZAJE\n\nSu solicitud ha sido receptada\n\nSus credenciales de acceso son:\n\nusuario: '+dato['Usuario'].lower()+'\ncontrasena: '+ str(pwd)+'\n\nPara empezar, ingrese en nuestro sitio: \n\n'+sitio+'\n:)'
                #contenido2=str(pwd)
                asunto='Credenciales de acceso al Repositorio de Objetos de Aprendizaje'

                send_mail( asunto, contenido, 'repositoriolibres@gmail.com', [dato['Email']], fail_silently=False,
                )
