...
def validar(request, id_proyecto):
    ...
    if puederevisar:
        ...
        if request.user.profile.rol == 'rrev' and obj.proyecto.fase != "f1":  #
            ...
            data = {'errores': l_errores, 'indicadores': l_indicadores, 'operaciones': l_operaciones, 'tipos': tipos,
                    'parametros': parametros, 'formValidar': f_validar, 'usuario': request.user, 'objeto': obj,
                    'espec': obj.espec_lom, 'autores': obj.autores.all(), 'keywords': obj.palabras_claves.all(),
