...
def Proyecto(request):
    ...
    if request.user.profile.rol == 'rcat':
                        ...
                        if formularioPro.is_valid():  # si el válido el proyecto
                            ...
                            for l in lpc:
                                p, b = PalabraClave.objects.get_or_create(
                                    palabra_clave=l)  # Se crea una palabra clave por cada palabra en la lista
        ...
        return render_to_response('proyecto.html',
                                  {'usuario': request.user, 'proyecto': proyectoObj, 'formObj': formularioObj,
                                   'formPro': formularioPro, 'formEsp': formularioEsp, 'errores': errores,
                                   'l_errores': l_errores}, context_instance=RequestContext(request))
    ...
    if puedever | obj.repositorio.publico:
        if request.user.is_authenticated():
            data = {'usuario': request.user, 'objeto': obj, 'espec': obj.espec_lom, 'autores': obj.autores.all(),
                    'keywords': obj.palabras_claves.all(), 'proyecto': obj.proyecto, 'indicadores': l_indicadores,
                    'operaciones': l_operaciones}
        else:
    ...
    if request.user.profile.rol == 'rcat':
        if obj.creador == request.user:
                    ...
                    if formularioEsp.is_valid() & formularioObj.is_valid() & formularioPro.is_valid():
                        ...
                        for kw in kws:  # Se recorre todo el conjunto de palabras claves del objeto
                            if kw.palabra_clave not in lpc:  # se valida si cada palabra clave todavía se mantiene en lo que el usuario digitó
                                f.palabras_claves.remove(
                                    kw)  # de no encontrarse la palabra clave debe desasociarse aunque no eliminarse.
                        for l in l_autores:  # como el objeto llega como una lista... se debe recorrer per en realidad siempre tiene un solo objeto
                        ...
                        for autor in autores:  # Se recorre todo el conjunto de autores del objeto
                            ...
                            if cadena_temporal not in lista_temporal:  # se valida si cada autor todavía se mantiene en lo que el usuario digitó
                                f.autores.remove(
                                    autor)  # de no encontrarse el autor, debe desasociarse aunque no eliminarse.
                        f.repositorio = re
                        f.proyecto = pro
                        f.save()
            ...
            return render_to_response('editProyecto.html',
                                      {'objeto': obj, 'usuario': request.user, 'formObj': formularioObj,
                                       'formEsp': formularioEsp, 'formPro': formularioPro, 'proyecto': obj.proyecto,
                                       'autores': autores, 'errores': errores, 'l_errores': l_errores},
...
def asociarProyecto(request, id_objeto):
    ...
    if request.user.profile.rol == 'rcat':
        ...
        if pro.fase == 'f3':
            l_errores.append(
                "El proyecto no puede ser asociado a estándares de competencia debido a que ya ha sido valorado")
            error = True
        else:
            for f in fac:
                if len(lenunciado) == 0:
        ...
        l_oind = pro.indicadores.all()
        operaciones2 = pro.operaciones.all()
        data = {'usuario': request.user, 'error': error, 'objeto': obj, 'proyecto': pro, 'espec': obj.espec_lom,
                'operaciones': operaciones, 'operaciones2': operaciones2, 'autores': obj.autores.all(),
                'keywords': obj.palabras_claves.all(), 'l_oind': l_oind, 'l_errores': l_errores, 'factor': fac,
                'lenunciado': lenunciado, 'lindicadores': lindicadores}
